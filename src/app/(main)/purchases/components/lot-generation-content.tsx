      ...
      <div className="w-full h-full flex flex-col page-break">
        ...
        <div className="flex-grow space-y-6">
          {items.map((item, index) => (
            <div key={index} style={{ border: '4px solid black', padding: '20px', margin: '10px', textAlign: 'center' }}>
              <h2 style={{ fontSize: '28px', margin: '0', fontWeight: 'bold' }}>ORDEN DE COMPRA: {item.orderId}</h2>
              <h3 style={{ fontSize: '24px', margin: '5px 0' }}>PRODUCTO: {item.productName.toUpperCase()}</h3>
              <h1 style={{ fontSize: '48px', margin: '0', fontWeight: 'bold' }}>CALIBRE: {item.caliber.toUpperCase()}</h1>
            </div>
          ))}
        </div>
      </div>
       ...
       <div className="w-full h-full flex flex-col items-center justify-center">
        {firstItem && (
            <div style={{ border: '2px solid black', padding: '20px', width: '400px', fontFamily: 'Arial, sans-serif' }}>
                ...
                <p style={{ fontSize: '32px', fontWeight: 'bold', margin: '10px 0', textAlign: 'center' }}>CALIBRE: {firstItem.caliber.toUpperCase()}</p>
